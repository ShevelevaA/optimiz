...
string TDiagram::Vyrazh(){
	...
	while ((t>=Type_More) && (t<=Type_No_Eq)) { // знаки сравнения стоят подряд
		...
		bool localFlConst = FlConst;
		DataType localType = GlobalType;

		switch (t)
		{
		case Type_Little:
			if(localFlConst && FlConst){
				if(localType == GlobalType && localType == type_int)
					data1 = to_string(atol(data1.c_str()) < atol(data2.c_str()));
			}
			else
		...
		if (t == Type_Plus)
		{
			...
			if(localFlConst && FlConst){
				if(localType == GlobalType && localType == type_int)
					...
					if(localFlConst == type_int)
					{
						data1 = to_string(atol(data1.c_str()) + atof(data2.c_str()));
						GlobalType = localType = type_double;
					}
					else
					{
						data1 = to_string(atof(data1.c_str()) + atol(data2.c_str()));
						GlobalType = localType = type_double;
					}
			}
			else
		...
		else
		{
			...
			if(localFlConst && FlConst)
			{
				if(localType == GlobalType && localType == type_int)
					...
					sc -> PrintError("Вычисление невозможно!", sc -> uk);
					/*if(localFlConst == type_int)
					{
						data1 = to_string(atol(data1.c_str()) % atof(data2.c_str()));
						GlobalType = localType = type_int;
					}
					else
					{
						data1 = to_string(atof(data1.c_str()) % atol(data2.c_str()));
